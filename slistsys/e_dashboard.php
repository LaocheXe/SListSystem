...
class slistsys_dashboard // include plugin-folder in the name.
{
	...
	function activity()
	{
		...
		if(!$sql->gen("SELECT slistsys_id,slistsys_date,slistsys_folder FROM `#slistsys` WHERE slistsys_date BETWEEN ".$month_start." AND ".$month_end))
		{
			return false;
		}
		...
		while($row = $sql->fetch())
		{
			...
			$key = date('Y-n-j', $row['slistsys_date']);

			switch($row['slistsys_folder'])
			{
				case "Facebook":
					$amt[$key]['facebook'] += 1;
					break;
			...
			$dateName[$key] = date('jS', $row['slistsys_date']);
		}
	...
	function status() // Status Panel in the admin area
	{
		...
		$var[0]['icon'] 	= "<img src='".e_PLUGIN."slistsys/images/slistsys_16.png' alt='' />";
		$var[0]['title'] 	= "My Title";
		$var[0]['url']		= e_PLUGIN_ABS."slistsys/slistsys.php";
		$var[0]['total'] 	= 10;
